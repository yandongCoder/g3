...
tape("Get json string of a Node.", function(test){
    ...
    test.deepEqual(myGraph.nodes()[0].getJSON(), {color: myGraph.config.color, icon:"", id:1, label: "", mugshot: '', x:10, y:10, radius:10, selected:false});
...
tape("Node's groupedBy property only maintain id array in json string.", function(test){
    ...
    test.deepEqual(myGraph.nodes()[3].getJSON(), {id:"grouped:1&2", icon: "", label:"&", mugshot: '', x:0, y:0, radius:15, color:"#359aff", selected:false, groupedBy:{nodes:[1,2], links:[1], attachedLinks: [{link:2, originalSource:2},{link:3, originalTarget:2}]}});
